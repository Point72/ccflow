...
from spaday_webawesome import Tabs, WaSwitch

import ccflow

from .graph import DEPENDENCY_RANKDIR_FIELD, dependency_graph_view
...
#: The signal-store field holding the active main-area tab.
VIEW_FIELD = "view"
...
def registry_store() -> dict:
    ...
    return {SELECTED_FIELD: "", VIEW_FIELD: "details", DARK_FIELD: False, DEPENDENCY_RANKDIR_FIELD: "LR"}
...
def registry_viewer(registry, *, title: str = "ccflow Model Registry", browser_width: int = 400, sort_children: bool = True) -> App:
    """Compose the full page: a sidebar registry tree, and the selected model's details or the graph."""
    ...
    tabs = Tabs(active="details").bind("active", VIEW_FIELD, mode="two-way")
    tabs.tab("Details", _details_view(leaves), name="details")
    tabs.tab("Dependencies", dependency_graph_view(leaves, selected_field=SELECTED_FIELD), name="dependencies")
