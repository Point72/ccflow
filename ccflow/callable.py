...
_DYNAMIC_CONTEXTS_MODULE_NAME = "ccflow._dynamic_contexts"
_DYNAMIC_CALLABLES_MODULE_NAME = "ccflow._dynamic_callable_models"


def _ensure_dynamic_module(name: str, doc: str) -> ModuleType:
    """Make sure a module exists in sys.modules so PyObjectPath can import runtime artifacts."""
...
_DYNAMIC_CONTEXTS_MODULE = _ensure_dynamic_module(
    _DYNAMIC_CONTEXTS_MODULE_NAME,
    "Runtime-only ContextBase subclasses generated by Flow.dynamic_call.",
)
_DYNAMIC_CALLABLES_MODULE = _ensure_dynamic_module(
    _DYNAMIC_CALLABLES_MODULE_NAME,
    "CallableModel subclasses defined outside importable modules.",
)

_dynamic_context_counter = count()
_dynamic_callable_counter = count()
...
def _register_in_module(*, cls: Type[Any], module: ModuleType, name_hint: str, fallback: str, counter: count) -> str:
    """Give cls a stable import path by attaching it to `module` under a synthesized name."""
    unique_name = f"{_sanitize_identifier(name_hint, fallback)}_{next(counter)}"
    setattr(module, unique_name, cls)
...
def _register_dynamic_context(cls: Type["ContextBase"], name_hint: str) -> None:
    """Place dynamic contexts in an importable module so caching/serialization can reload them."""
    _register_in_module(
        cls=cls,
        module=_DYNAMIC_CONTEXTS_MODULE,
        name_hint=name_hint,
        fallback="DynamicContext",
        counter=_dynamic_context_counter,
    )


def _maybe_register_callable_subclass(cls: Type["_CallableModel"]) -> None:
    """Register CallableModels created in local scopes (e.g. inside tests or notebooks)."""
    if getattr(cls, "__module__", "").startswith(_DYNAMIC_CALLABLES_MODULE_NAME):
        return
    qualname = getattr(cls, "__qualname__", "")
    module = getattr(cls, "__module__", "")
    if "<locals>" not in qualname and module != "__main__":
        return
    name_hint = f"{module}.{qualname}"
    _register_in_module(
        cls=cls,
        module=_DYNAMIC_CALLABLES_MODULE,
        name_hint=name_hint,
        fallback="CallableModel",
        counter=_dynamic_callable_counter,
    )
...
class _CallableModel(BaseModel, abc.ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        _maybe_register_callable_subclass(cls)
...
def dynamic_context(func: Callable = None, *, parent: Type[ContextBase] = None) -> Callable:
    ...
    _register_dynamic_context(dyn_context, context_name)

    @wraps(func)
    def wrapper(self, context: dyn_context) -> result_type:  # noqa: F821
        # Only pass signature-defined fields; parent context data stays on the context object.
        fn_kwargs = {name: getattr(context, name) for name in field_names}
